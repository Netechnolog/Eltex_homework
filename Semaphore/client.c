...
int main(void){
    ...
    char* str_1 = mmap(NULL, MSG_SIZE, PROT_WRITE, MAP_SHARED, shm_first, 0);
    if(str_1 == MAP_FAILED)
    {
    ...
    char* str_2 = mmap(NULL, MSG_SIZE, PROT_READ, MAP_SHARED, shm_second, 0);
    if(str_2 == MAP_FAILED)
    {
    ...
    sem_post(sem_2);
    sem_close(sem_1);
    sem_unlink(PATCH_SEM_2);
    munmap(str_1, MSG_SIZE);
    munmap(str_2, MSG_SIZE);
    shm_unlink(PATCH_SHM_2);
